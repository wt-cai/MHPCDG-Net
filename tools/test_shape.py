...
if __name__ == '__main__':
    ...
    for i, v in enumerate(imgs_path):
        ...
        if v.endswith('.mat'):
            ...
            # path_mat = osp.join(BASEDIR, v)
            path_mat = osp.join(os.path.dirname(os.path.dirname(__file__)), v)
            hy_data = h5py.File(path_mat)
            # 注意，读取出来的hy_data是字典格式，可以通过函数type(data)查看。
            print("hy_shape:",hy_data['cube'].shape)
            print("hy_dim:",hy_data['cube'].shape[1],hy_data['cube'].shape[2])
            hy_weight=hy_data['cube'].shape[1]
            hy_height=hy_data['cube'].shape[2]
            print("hy_shape:",hy_weight,hy_height)
            for k in hy_data:
                print(f'{k}:\n{hy_data[k]}')
        else:
            #pass
            ...
            rgb = cv2.resize(rgb, (hy_weight,hy_height))
            ...
            reconstruct_depth(depth_scaleinv, rgb[:, :, ::-1], hy_data , image_dir_out, img_name[:-4]+'-pcd', focal = focal_length)
